...
class Utterance:
    ...
    def process_gold_seq(self,
        ...
        # Assign learning weights
        self.gold_query_weights = [1.0] * (len(self.gold_query_to_use) + 1) # EOS
    ...
    def set_gold_query_weights(self, weights): # added by Ziyu
        self.gold_query_weights = weights
        # assert len(self.gold_query_weights) == len(self.gold_query_to_use)
        # revise 0209:
        assert len(self.gold_query_weights) == len(self.gold_query_to_use) + 1 # one EOS
